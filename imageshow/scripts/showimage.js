...
function showZoomValue(imgobj) {
	let zv = '<br>zoom:' + Math.round(100 / imgobj.ratio) + '%';
...
function showExif(imgobj) {
	...
	for (let i = 0; i < exifkey.length; i++) {
		let exifvalue = '--';
			if (imgobj.exif[exifkey[i]])
				if (exifkey[i] === 'ExposureTime')
					exifvalue = '1/' + imgobj.exif[exifkey[i]]['denominator'];
				else
					exifvalue = imgobj.exif[exifkey[i]];
			imgexif = `${imgexif}<br>${exifkey[i]}: ${exifvalue}`;				
	}
	imgobj.innerHTML = `${imgobj.innerHTML}${imgexif}`;
}
...
function updateBackroundImage(imgobj, fname) {
	...
	EXIF.getData(fname, function() {
		imgobj.exif = EXIF.getAllTags(this); });
}
